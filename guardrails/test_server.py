        ...
        if response.status_code == 200:
            ...
            if expected_validation is not None:
                if validation_passed == expected_validation:
                    print(
                        f"✓ Validation result matches expected: {expected_validation}"
                    )
                else:
                    print(
                        f"✗ Validation result mismatch. Expected: {expected_validation}, Got: {validation_passed}"
                    )
...
def main():
    ...
    if success_count == total_tests:
        ...
        print(
            f"⚠️  {total_tests - success_count} tests failed. Please check the server configuration."
        )
