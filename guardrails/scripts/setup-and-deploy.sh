...
if [ -z "$GUARDRAILS_TOKEN" ]; then
    ...
    echo "Please set it by running: export GUARDRAILS_TOKEN=your_token_here"
    exit 1
fi

# Build the container
echo "Building container..."
gcloud builds submit --config cloudbuild.yaml --substitutions=_GUARDRAILS_TOKEN="$GUARDRAILS_TOKEN"

# Deploy to Cloud Run with secrets and non-sensitive env vars
echo "Deploying to Cloud Run..."
gcloud run deploy guardrails \
  --image gcr.io/arize-461218/guardrails \
  --platform managed \
  --region us-central1 \
