...
def has_valid_arize_config(
    ...
    effective_config["model_id"] = get_non_empty_value(
        "ARIZE_MODEL_ID", "ARIZE_MODEL_ID", "langgraph_finance_agent"
    )
...
def initialize_langgraph_app(env_overrides: Optional[Dict[str, str]] = None):
    ...
    try:
        ...
        components = {"langgraph_app": langgraph_app, "tracer": tracer}
        ...
        logger.info(
            f"LangGraph Finance Agent initialized successfully with env_overrides: {list(env_overrides.keys()) if env_overrides else 'default'}"
        )
...
app_state = {"initialized": False, "langgraph_app": None, "tracer": None}
...
async def finance_query(request: FinanceQueryRequest):
    ...
    with EnvironmentManager.temporary_env_vars(env_overrides):
        ...
        try:
            ...
            return FinanceQueryResponse(result=final_result, thread_id=thread_id)
...
async def status():
    ...
    try:
        ...
        has_default_components = (
            hasattr(app.state, "default_components")
            and app.state.default_components is not None
        )
        ...
        return {
            ...
            "service": "langgraph-finance-agent",
            "cache_stats": {"cached_configurations": len(session_manager._cache)},
        }
    except Exception as e:
