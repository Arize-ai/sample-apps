...
class QueryType(BaseModel):
    category: str = Field(
        description="The category of the query: 'american_airlines', or 'out_of_scope'"
    )
...
class QueryClassifier:
    ...
    def get_response(self, query: str, category: QueryCategory, span=None) -> Response:
        try:
            if category == QueryCategory.AMERICAN_AIRLINES:
                try:
                    ...
                    logger.error(
                        f"Error in American Airlines response generation: {str(e)}"
                    )
                    raise
            else:
                return Response(
