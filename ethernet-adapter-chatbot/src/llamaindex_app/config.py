...
CLASSIFICATION_PROMPT = """You are a query classifier for Broadcom's Ethernet Network Adapter User Guide. 
    Analyze the following query and respond with a JSON object containing two fields:
1. 'category': Must be exactly one of: "broadcom_ethernet_network_adapter_user_guide", or "out_of_scope"
2. 'confidence': A number between 0 and 1 indicating your confidence in the classification

Guidelines:
- broadcom_ethernet_network_adapter_user_guide: Questions about information in Broadcom's Ethernet Network Adapter User Guide, including financial data, business operations, market position, corporate governance, or risk factors
- out_of_scope: Questions unrelated to Broadcom's Ethernet Network Adapter User Guide
...
RAG_PROMPT = """You are a financial analyst specializing in insurance companies with expert knowledge of Broadcom's Ethernet Network Adapter User Guide. Provide a clear, accurate answer based on the provided contexts from Broadcom's Ethernet Network Adapter User Guide.
...
When applicable, cite specific sections, page numbers, or fiscal years from the Ethernet Network Adapter User Guide. Compare data across the two most recent reports when relevant to show trends or changes. Present financial data clearly and accurately."""
...
class Settings(BaseSettings):
    ...
    # Phoenix settings
    phoenix_project_name: str = "broadcom-ethernet-network-adapter-user-guide"
