...
def example_runtime_configuration():
    ...
    config = TracerConfig(
        ...
        additional_attributes={"environment": "production", "version": "1.0.0"},
    )
...
def example_reconfiguration():
    ...
    config1 = TracerConfig(space_id="space_1", api_key="key_1", model_id="model_v1")
    ...
    config2 = TracerConfig(
        ...
        api_key="key_2",
        model_id="model_v2",
        additional_attributes={"experiment": "A/B_test_variant_B"},
    )
...
def example_temporary_configuration():
    ...
    temp_config = TracerConfig(
        ...
        additional_attributes={"debug_mode": True, "log_level": "verbose"},
    )
...
def example_cleanup():
    ...
    try:
        # Configure and use
        config = TracerConfig(
            space_id="cleanup_space", api_key="cleanup_key", model_id="cleanup_model"
        )
        manager.configure(config)
