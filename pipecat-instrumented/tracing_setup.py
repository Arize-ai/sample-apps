...
def patch_pipecat_span_creation():
    ...
    try:
                ...
                with trace_api.use_span(current_span):
                    with tracer.start_as_current_span(
                        ...
                        attributes={
                            ...
                            SpanAttributes.INPUT_VALUE: text[
                                :500
                            ],  # Truncate long text
        ...
        async def traced_handle_transcription(
            ...
            if not tracer:
                # Fallback to original if no tracer
                return await original_handle_transcription(
                    self, transcript, is_final, language
                )
            ...
            if current_span and current_span.is_recording():
                with trace_api.use_span(current_span):
                    with tracer.start_as_current_span(
                        ...
                        attributes={
                            SpanAttributes.OPENINFERENCE_SPAN_KIND: OpenInferenceSpanKindValues.CHAIN.value,
                            SpanAttributes.OUTPUT_VALUE: transcript[
                                :500
                        ...
                        try:
                            ...
                            result = await original_handle_transcription(
                                self, transcript, is_final, language
                            )
        ...
        logger.info(
            "✅ Successfully patched TTS and STT services for manual span creation"
        )
...
def setup_arize_tracing():
    ...
    try:
        ...
        logger.info("🔭 Initializing Arize AX Tracing (Native Mode) 🔭")
        logger.info(f"|  Project: {project_name}")
        logger.info(f"|  Development Mode: {is_development}")
        logger.info("|  Mode: OpenInference + Native Pipecat spans")
        ...
        logger.info(
            "🎯 Manual span creation mode: Create spans for every TTS/STT operation"
        )
