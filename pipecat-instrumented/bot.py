...
from pipecat.transports.services.helpers.daily_rest import DailyRESTHelper, DailyRoomObject, DailyRoomParams
        ...
        async with aiohttp.ClientSession() as aiohttp_session:
            ...
            try:
                ...
                with tts_span:
                    ...
                    trace_audio_processing("tts_service_initialized", {
                        "service": "elevenlabs",
                        "voice_id": os.getenv("ELEVENLABS_VOICE_ID"),
                        "model": os.getenv("ELEVENLABS_MODEL")
                    })
                ...
                with stt_span:
                    stt = DeepgramSTTService(api_key=os.getenv("DEEPGRAM_API_KEY"), model="nova-2", language="en")
                    # STT operations will be traced via manual span creation
                    
                    trace_audio_processing("stt_service_initialized", {
                        "service": "deepgram",
                        "model": "nova-2",
                        "language": "en"
                    })

                context_span = create_child_span_with_context("context_aggregator_init", "CHAIN") 
                with context_span:
                    ...
                    trace_pipeline_event("context_aggregator_initialized", message_count=1)

                # Create pipeline with tracing
                pipeline_init_span = create_child_span_with_context("pipeline_creation", "CHAIN")
                ...
                runner_span = create_child_span_with_context("pipeline_execution", "CHAIN")
                ...
                try:
                    with runner_span:
                        ...
                        trace_pipeline_event("pipeline_task_created", allow_interruptions=True)
...
async def local_daily():
    ...
    with SessionTracer(session_id, "local_development") as session:
        ...
        try:
            ...
            config_span = create_child_span_with_context("daily_room_configuration", "CHAIN")
            with config_span:
                async with aiohttp.ClientSession() as aiohttp_session:
            ...
            transport_span = create_child_span_with_context("transport_initialization", "CHAIN")
            with transport_span:
                ...
                trace_pipeline_event("transport_initialized", 
                    room_url=room_url,
                    bot_name="Pipecat Local Bot"
                )
...
async def main_daily(room_url: str):
    ...
    with SessionTracer(session_id, "production") as session:
        ...
        try:
            transport_span = create_child_span_with_context("production_transport_init", "CHAIN")
            with transport_span:
                transport = DailyTransport(
                    room_url,
                ...
                add_session_metadata(
                    bot_name="Pipecat Production Bot",
...
async def bot(args):
    ...
    with SessionTracer(session_id, "pipecat_cloud") as session:
        ...
        try:
            transport_span = create_child_span_with_context("cloud_transport_init", "CHAIN")
            with transport_span:
                ...
                trace_pipeline_event("cloud_transport_initialized",
                    room_url=args.room_url,
                    bot_name="Pipecat Cloud Bot"
                )
...
if LOCAL_RUN and __name__ == "__main__":
    ...
    if not daily_room_url:
        ...
        logger.error("  Example: DAILY_ROOM_URL=https://your-room.daily.co python bot.py")
        logger.error("")
